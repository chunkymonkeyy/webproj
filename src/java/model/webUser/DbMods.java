...
public class DbMods {
    ...
    public static StringData newFind (DbConn dbc, String user_email, String user_password) {
        ...
        if((user_email == null) || (user_password == null)){ //
            sd.errorMsg = "Error - Must provide a vaild user email and a valid password for search";
            return sd;
        }
        ...
        try {
            String sql = "SELECT web_user_id, user_email, user_password, membership_fee, birthday, image, "
                    + "web_user.user_role_id, user_role_type "
                    + "FROM web_user, user_role WHERE web_user.user_role_id = user_role.user_role_id "
                    + "AND user_email= ? AND user_password= ?"; //email= & password=
            ...
            if (results.next()) { // id is unique, one or zero records expected in result set
                ...
                sd.errorMsg = "Web User Not Found. Please check your Email and Password";
            }
            results.close();
            stmt.close();
        } catch (Exception e) {
    ...
    public static String delete(String userId, DbConn dbc) {
        ...
        if (userId == null) {
            return "Error in model.webUser.DbMods.delete: cannot delete web_user record because 'userId' is null";
        }
